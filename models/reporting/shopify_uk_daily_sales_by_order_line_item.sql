{{ config (
    alias = target.database + '_shopify_uk_daily_sales_by_order_line_item',
...
{%- set schema_name,
        ...
        = 'shopify_raw_uk', 'customer_tag', 'product_tag'-%}
...
WITH orders AS 
    ...
    line_items AS 
    (SELECT *
    FROM {{ ref('shopify_uk_line_items') }}
    ),
